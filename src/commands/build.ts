...
export function runBuild(root: string, options: BuildOptions): BuildSummary {
  ...
  for (const file of output.files) {
    ...
    if (existsSync(abs)) {
      ...
      // Never clobber a file arcana did not generate — repos adopting arcana
      // usually have a hand-written CLAUDE.md whose content must be moved, not lost.
      if (!isStamped(existing)) {
        throw new ArcanaError(
          `${file.path} already exists and was not generated by arcana — move its ` +
            'content into your sources (e.g. deck.yaml conduct bindings or src/precepts.md), '
